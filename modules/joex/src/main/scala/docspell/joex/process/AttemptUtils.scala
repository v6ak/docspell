...
object AttemptUtils {
  def attemptTraverseAttachments[F[_]: Async, O]
    (o: Any, item: ItemData)
    (f: RAttachment => F[O]):
  F[(Vector[O], Seq[Throwable])] =// {
    item.attachments.traverse(attemptAttachment(o)(f)).map(all =>
      (onlySuccesses(all), onlyErrors(all))
    )
  ...
  /** Essentially like function.attempt, but with extra details when it fails.
    ...
    */
  def attemptAttachment[F[_]: Async, O](o: Any)(f: RAttachment => F[O]): RAttachment => F[Either[Throwable, O]] =
    ra =>
      f(ra)
        .attempt
        .map(
